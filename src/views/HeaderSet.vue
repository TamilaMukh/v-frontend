<template>
    ...
    <div class="w-1/2 lg:w-full bg-lghtgr">
      <div class="w-11/12 mx-auto p-5 items-center justify-between flex">
        ...
        <div class="w-1/3 h-1 bg-prpl rounded-xl"></div>
        ...
        <div v-if="showTown === 1" class="thrdDrop w-trn p-3 bg-neutral-100 text-center rounded-lg">
          <p class="mx-1 p-3 cursor-pointer hover:text-prpl border-b">Astana</p>
          <p class="mx-1 p-3 cursor-pointer hover:text-prpl border-b">Uralsk</p>
          <p class="mx-1 p-3 cursor-pointer hover:text-prpl border-b">Pavlodar</p>
        </div>
        <p class="hover:text-prpl transition"><router-link to="/products">Catalogue</router-link></p>
        <p @click="changeM()" class="cursor-pointer hover:text-prpl transition">Log in</p>
        <i @click="changeF()" class="far fa-heart text-xl cursor-pointer hover:text-prpl transition"></i>
        <i @click="changeC()" class="fas fa-shopping-cart text-xl cursor-pointer hover:text-prpl transition"></i>
      </div>
    </div>
    <div v-if="showFavorites === 1" class="frstDrop w-trn p-3 bg-neutral-100 text-center rounded-lg"><p class="text-prpl font-semibold">favorites</p>
    <p>...</p></div>
    <div v-if="showCart === 1" class="scndDrop w-trn p-3 bg-neutral-100 text-center rounded-lg"><p class="text-prpl font-semibold">cart</p>
    <p>...</p></div>

    <div v-if="showModal === 1">
      <div class="max-h-screen py-10 bg-black/75 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0" id="modal">
        <div v-if="logIn === 1" role="login" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
          <div class="relative py-8 px-5 md:px-10 bg-white rounded-lg">
